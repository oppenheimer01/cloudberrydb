...
/* no foreign keys */
DECLARE_UNIQUE_INDEX(pg_resgroup_oid_index, 6447, ResGroupOidIndexId, pg_resgroup, on pg_resgroup using btree(oid oid_ops));
DECLARE_UNIQUE_INDEX(pg_resgroup_rsgname_index, 6444, ResGroupRsgnameIndexId, pg_resgroup, on pg_resgroup using btree(rsgname name_ops));
/* ----------------
...
typedef enum ResGroupLimitType
{
	...
	RESGROUP_LIMIT_TYPE_MEMORY_QUOTA,		/* memory_quota */

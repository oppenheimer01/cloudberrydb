...
UPDATE x1 SET b = b || '_updt' WHERE f_leak(b) RETURNING *;
-- start_ignore
-- PAX does not support TupleFetchRowVersion, so DELETE fails with ERROR.
-- The NOTICE messages before the ERROR are non-deterministic because
-- the number of rows processed before hitting the error varies.
